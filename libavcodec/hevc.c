...
static int set_sps(HEVCContext *s, const HEVCSPS *sps)
{
    ...
    if (s->ps.vps->vps_timing_info_present_flag) {
        num = s->ps.vps->vps_num_units_in_tick;
        den = s->ps.vps->vps_time_scale;
    ...
    if (!sh->dependent_slice_segment_flag) {
        ...
        if(!IS_IDR(s)) {
            ...
            if (!short_term_ref_pic_set_sps_flag) {
                ret = ff_hevc_decode_short_term_rps(s, &sh->slice_rps, s->ps.sps, 1);
                if (ret < 0)
                    return ret;
                               ...
                               int log2_cb_size, int cb_depth)
{
    ...
    if (x0 + cb_size <= s->ps.sps->width  &&
        ...
        split_cu_flag = (log2_cb_size > s->ps.sps->log2_min_cb_size);
    }
    if (s->ps.pps->cu_qp_delta_enabled_flag &&
...
//careful here segfault
    //if(s->decoder_id == 1 && s->ps.vps && s->ps.vps->vps_base_layer_internal_flag == 0 && s->ps.vps->vps_base_layer_available_flag == 1)
            //av_freep(&s->BL_frame);
