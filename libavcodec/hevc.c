...
static int hls_slice_header(HEVCContext *s)
{
    ...
    s->HEVClc->ca.first_qp_group = !s->sh.dependent_slice_segment_flag;
...
static void hls_sao_param(HEVCContext *s, int rx, int ry)
{
    ...
    if (s->sh.slice_sample_adaptive_offset_flag[0] ||
        ...
        if (rx > 0) {
            if (lc_ca->ctb_left_flag)
                sao_merge_left_flag = ff_hevc_sao_merge_flag_decode(s);
        }
        if (ry > 0 && !sao_merge_left_flag) {
            if (lc_ca->ctb_up_flag)
                              ...
                              int trafo_depth, int blk_idx)
{
    ...
    if (SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0) ||
        ...
        if (s->pps->cu_qp_delta_enabled_flag && !lc_ca->tu.is_cu_qp_delta_coded) {
            lc_ca->tu.cu_qp_delta = ff_hevc_cu_qp_delta_abs(s);
            if (lc_ca->tu.cu_qp_delta != 0)
                if (ff_hevc_cu_qp_delta_sign_flag(s) == 1)
                    lc_ca->tu.cu_qp_delta = -lc_ca->tu.cu_qp_delta;
            lc_ca->tu.is_cu_qp_delta_coded = 1;

            if (lc_ca->tu.cu_qp_delta < -(26 + s->sps->qp_bd_offset / 2) ||
                    lc_ca->tu.cu_qp_delta >  (25 + s->sps->qp_bd_offset / 2)) {
                av_log(s->avctx, AV_LOG_ERROR,
                       "The cu_qp_delta %d is outside the valid range "
                       "[%d, %d].\n",
                       lc_ca->tu.cu_qp_delta,
        ...
        if (lc->cu.pred_mode == MODE_INTRA && log2_trafo_size < 4) {
            if (lc_ca->tu.cur_intra_pred_mode >= 6 &&
            ...
            if (lc_ca->tu.cur_intra_pred_mode_c >=  6 &&
...
static int hls_transform_unit_cabac(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx)
{
    ...
    SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], x0, y0 + (1 << log2_trafo_size_c)) = SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], x0, y0 + (1 << log2_trafo_size_c));
    for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
        SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)) = SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth], xBase, yBase + (i << log2_trafo_size_c));
        SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)) = SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], xBase, yBase + (i << log2_trafo_size_c));
    }
    ...
    if (SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0) ||
        ...
        if (SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0))
            ff_hevc_hls_residual_coding_cabac(s, x0, y0, log2_trafo_size, scan_idx, 0, trafo_depth, blk_idx);
        if (log2_trafo_size > 2 || s->sps->chroma_array_type == 3) {
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], x0, y0 + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_cabac(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, scan_idx_c, 1, trafo_depth, blk_idx);
            }
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], x0, y0 + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_cabac(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, scan_idx_c, 2, trafo_depth, blk_idx);
            }
        } else if (blk_idx == 3) {
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_cabac(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, scan_idx_c, 1, trafo_depth, blk_idx);
            }
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_cabac(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, scan_idx_c, 2, trafo_depth, blk_idx);
            }
        }
    }
...
static int hls_transform_unit_compute(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx)
{
    ...
    if (SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0) ||
        ...
        if (SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0))
            ff_hevc_hls_residual_coding_compute(s, x0, y0, log2_trafo_size, 0, trafo_depth, blk_idx);
        if (log2_trafo_size > 2 || s->sps->chroma_array_type == 3) {
            int trafo_size_h = 1 << (log2_trafo_size_c + s->sps->hshift[1]);
            int trafo_size_v = 1 << (log2_trafo_size_c + s->sps->vshift[1]);
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                ...
                if (SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], x0, y0 + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_compute(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, 1, trafo_depth, blk_idx);
            }
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (lc->cu.pred_mode == MODE_INTRA) {
                    ff_hevc_set_neighbour_available(s, x0, y0 + (i << log2_trafo_size_c), trafo_size_h, trafo_size_v);
                ...
                if (SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], x0, y0 + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_compute(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, 2, trafo_depth, blk_idx);
            }
        } else if (blk_idx == 3) {
            int trafo_size_h = 1 << (log2_trafo_size + 1);
            int trafo_size_v = 1 << (log2_trafo_size + s->sps->vshift[1]);
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                ...
                if (SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_compute(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, 1, trafo_depth, blk_idx);
            }
            for (i = 0; i < (s->sps->chroma_array_type  ==  2 ? 2 : 1 ); i++ ) {
                if (lc->cu.pred_mode == MODE_INTRA) {
                    ff_hevc_set_neighbour_available(s, xBase, yBase + (i << log2_trafo_size),
                ...
                if (SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], xBase, yBase + (i << log2_trafo_size_c)))
                    ff_hevc_hls_residual_coding_compute(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, 2, trafo_depth, blk_idx);
            }
        }
    } else if (lc->cu.pred_mode == MODE_INTRA) {
        if (log2_trafo_size > 2 || s->sps->chroma_array_type == 3) {
...
static int hls_transform_unit2(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx) {
#if 1
    hls_transform_unit_cabac(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase,
            log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
    return hls_transform_unit_compute(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase,
            log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
#else
    return hls_transform_unit(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase,
            log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
#endif
}
                              ...
                              int trafo_depth, int blk_idx)
{
    ...
    if (trafo_depth > 0 && log2_trafo_size == 2) {
        ...
        if (s->sps->chroma_array_type == 2) {
            ...
            SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) =
                SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth - 1], xBase_cb, yBase_cb + (1 << (log2_trafo_size)));
            SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) =
                SAMPLE_CBF(lc->tt.cbf_cr[trafo_depth - 1], xBase_cb, yBase_cb + (1 << (log2_trafo_size)));
        }
    } else {
        SAMPLE_CBF(lc->tt.cbf_cb[trafo_depth], x0, y0) =
    ...
    if (lc_ca->cu.intra_split_flag) {
        if (trafo_depth == 1) {
            lc_ca->tu.cur_intra_pred_mode   = lc_ca->pu.intra_pred_mode[blk_idx];
            if (s->sps->chroma_array_type == 3)
                lc_ca->tu.cur_intra_pred_mode_c = lc_ca->pu.intra_pred_mode_c[blk_idx];
            else
                lc_ca->tu.cur_intra_pred_mode_c = lc_ca->pu.intra_pred_mode_c[0];
        }
    } else {
        lc_ca->tu.cur_intra_pred_mode   = lc_ca->pu.intra_pred_mode[0];
        lc_ca->tu.cur_intra_pred_mode_c = lc_ca->pu.intra_pred_mode_c[0];
    }

    SAMPLE_CBF(lc->tt.cbf_luma[trafo_depth], x0, y0) = 1;

    inter_split_flag = s->sps->max_transform_hierarchy_depth_inter == 0 &&
    ...
    if (log2_trafo_size <= s->sps->log2_max_trafo_size &&
        ...
        trafo_depth     < lc_ca->cu.max_trafo_depth       &&
        !(lc_ca->cu.intra_split_flag && trafo_depth == 0)) {
    ...
    if (split_transform_flag) {
        ...
        ret = hls_transform_unit2(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase,
...
static int hls_transform_tree_cabac(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx)
{
    ...
    if (trafo_depth > 0 && log2_trafo_size == 2) {
        SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth], x0, y0) =
            SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth - 1], xBase, yBase);
        SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], x0, y0) =
            SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth - 1], xBase, yBase);
        if (s->sps->chroma_array_type == 2) {
            int xBase_cb = xBase & ((1 << log2_trafo_size) - 1);
            int yBase_cb = yBase & ((1 << log2_trafo_size) - 1);
            SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) =
                SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth - 1], xBase_cb, yBase_cb + (1 << (log2_trafo_size)));
            SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) =
                SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth - 1], xBase_cb, yBase_cb + (1 << (log2_trafo_size)));
        }
    } else {
        SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth], x0, y0) =
        SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], x0, y0) = 0;
        if (s->sps->chroma_array_type == 2) {
                SAMPLE_CBF(lc_ca->tt.cbf_cb[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) =
                SAMPLE_CBF(lc_ca->tt.cbf_cr[trafo_depth], x0, y0 + (1 << (log2_trafo_size - 1))) = 0;
        }
    }
    ...
    SAMPLE_CBF(lc->tt.cur_intra_pred_mode[trafo_depth], x0, y0)   = lc_ca->tu.cur_intra_pred_mode;
    SAMPLE_CBF(lc->tt.cur_intra_pred_mode_c[trafo_depth], x0, y0) = lc_ca->tu.cur_intra_pred_mode_c;
    ...
    SAMPLE_CBF(lc->tt.split_transform_flag[trafo_depth], x0, y0) = split_transform_flag;
    ...
    if (split_transform_flag) {
        ...
        ret = hls_transform_tree_cabac(s, x0, y0, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 0);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_cabac(s, x1, y0, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 1);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_cabac(s, x0, y1, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 2);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_cabac(s, x1, y1, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 3);
        if (ret < 0)
            return ret;
...
static int hls_transform_tree_compute(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx)
{
    ...
    uint8_t split_transform_flag = SAMPLE_CBF(lc->tt.split_transform_flag[trafo_depth], x0, y0);
    int ret;

    lc_co->tu.cur_intra_pred_mode   = SAMPLE_CBF(lc->tt.cur_intra_pred_mode[trafo_depth], x0, y0);
    lc_co->tu.cur_intra_pred_mode_c = SAMPLE_CBF(lc->tt.cur_intra_pred_mode_c[trafo_depth], x0, y0);

    if (split_transform_flag) {
        int x1 = x0 + ((1 << log2_trafo_size) >> 1);
        int y1 = y0 + ((1 << log2_trafo_size) >> 1);

        ret = hls_transform_tree_compute(s, x0, y0, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 0);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_compute(s, x1, y0, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 1);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_compute(s, x0, y1, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 2);
        if (ret < 0)
            return ret;
        ret = hls_transform_tree_compute(s, x1, y1, x0, y0, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size - 1,
                                 trafo_depth + 1, 3);
        if (ret < 0)
            return ret;
        ...
        int log2_min_tu_size = s->sps->log2_min_tb_size;
        int min_tu_width     = s->sps->min_tb_width;
        ret = hls_transform_unit_compute(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase,
                                 log2_cb_size, log2_trafo_size, trafo_depth,
                                 blk_idx);
        if (ret < 0)
            return ret;
...
static int hls_transform_tree2(HEVCContext *s, int x0, int y0,
                              int xBase, int yBase, int cb_xBase, int cb_yBase,
                              int log2_cb_size, int log2_trafo_size,
                              int trafo_depth, int blk_idx)
{
#if 1
    hls_transform_tree_cabac(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
    return hls_transform_tree_compute(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
#else
    return hls_transform_tree(s, x0, y0, xBase, yBase, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size, trafo_depth, blk_idx);
#endif
}
                                ...
                                int prev_intra_luma_pred_flag)
{
    ...
    int cand_up   = (lc_ca->ctb_up_flag || y0b) ?
                    s->tab_ipm[(y_pu - 1) * min_pu_width + x_pu] : INTRA_DC;
    int cand_left = (lc_ca->ctb_left_flag || x0b) ?
    ...
    if (prev_intra_luma_pred_flag) {
        intra_pred_mode = candidate[lc_ca->pu.mpm_idx];
    } else {
        ...
        intra_pred_mode = lc_ca->pu.rem_intra_luma_pred_mode;
        for (i = 0; i < 3; i++)
    ...
    for (i = 0; i < side; i++) {
        for (j = 0; j < side; j++) {
            if (prev_intra_luma_pred_flag[2 * i + j])
                lc_ca->pu.mpm_idx = ff_hevc_mpm_idx_decode(s);
            else
                lc_ca->pu.rem_intra_luma_pred_mode = ff_hevc_rem_intra_luma_pred_mode_decode(s);

            lc_ca->pu.intra_pred_mode[2 * i + j] =
    ...
    if (s->sps->chroma_array_type  ==  3) {
        for (i = 0; i < side; i++) {
            for (j = 0; j < side; j++) {
                ...
                if (chroma_mode != 4) {
                    if (lc_ca->pu.intra_pred_mode[2 * i + j] == intra_chroma_table[chroma_mode])
                        lc_ca->pu.intra_pred_mode_c[2 * i + j] = 34;
                    else
                        lc_ca->pu.intra_pred_mode_c[2 * i + j] = intra_chroma_table[chroma_mode];
                } else {
                    lc_ca->pu.intra_pred_mode_c[2 * i + j] = lc_ca->pu.intra_pred_mode[2 * i + j];
                }
            }
        }
        ...
        chroma_mode = ff_hevc_intra_chroma_pred_mode_decode(s);
        if (chroma_mode != 4) {
            if (lc_ca->pu.intra_pred_mode[0] == intra_chroma_table[chroma_mode])
                mode_idx = 34;
            else
                mode_idx = intra_chroma_table[chroma_mode];
        } else {
            mode_idx = lc_ca->pu.intra_pred_mode[0];
        }
        lc_ca->pu.intra_pred_mode_c[0] = tab_mode_idx[mode_idx];
    } else if (s->sps->chroma_array_type  !=  0) {
        chroma_mode = ff_hevc_intra_chroma_pred_mode_decode(s);
        if (chroma_mode != 4) {
            if (lc_ca->pu.intra_pred_mode[0] == intra_chroma_table[chroma_mode])
                lc_ca->pu.intra_pred_mode_c[0] = 34;
            else
                lc_ca->pu.intra_pred_mode_c[0] = intra_chroma_table[chroma_mode];
        } else {
            lc_ca->pu.intra_pred_mode_c[0] = lc_ca->pu.intra_pred_mode[0];
        }
    }
}
...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
    ...
    lc->cu.part_mode        = PART_2Nx2N;
    lc_ca->cu.intra_split_flag = 0;
    lc->cu.pcm_flag         = 0;

    SAMPLE_CTB(s->skip_flag, x_cb, y_cb) = 0;
    for (x = 0; x < 4; x++)
        lc_ca->pu.intra_pred_mode[x] = 1;
    if (s->pps->transquant_bypass_enable_flag) {
    ...
    if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) {
        ...
        if (lc->cu.pred_mode != MODE_INTRA ||
            ...
            lc_ca->cu.intra_split_flag = lc->cu.part_mode == PART_NxN &&
                                      lc->cu.pred_mode == MODE_INTRA;
        }
        ...
        if (!lc->cu.pcm_flag) {
            ...
            if (lc->cu.rqt_root_cbf) {
                lc_ca->cu.max_trafo_depth = lc->cu.pred_mode == MODE_INTRA ?
                                         s->sps->max_transform_hierarchy_depth_intra + lc_ca->cu.intra_split_flag :
                                         s->sps->max_transform_hierarchy_depth_inter;
                ret = hls_transform_tree2(s, x0, y0, x0, y0, x0, y0,
                                         log2_cb_size,
                                         log2_cb_size, 0, 0);
                if (ret < 0)
                    return ret;
    ...
    if (s->pps->cu_qp_delta_enabled_flag && lc_ca->tu.is_cu_qp_delta_coded == 0)
    ...
    set_ct_depth(s, x0, y0, log2_cb_size, lc_ca->ct.depth);
...
static int hls_coding_unit_cabac(HEVCContext *s, int x0, int y0, int log2_cb_size, int cb_depth)
{
    ...
    if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) {
        ...
        if (lc->cu.pred_mode == MODE_INTRA) {
            ...
            case PART_2Nx2N:
                ...
                break;
            case PART_2NxN:
                hls_prediction_unit_cabac(s, x0, y0,               cb_size, cb_size / 2, log2_cb_size, 0, idx);
                hls_prediction_unit_cabac(s, x0, y0 + cb_size / 2, cb_size, cb_size / 2, log2_cb_size, 1, idx);
                break;
            case PART_Nx2N:
                hls_prediction_unit_cabac(s, x0,               y0, cb_size / 2, cb_size, log2_cb_size, 0, idx - 1);
                hls_prediction_unit_cabac(s, x0 + cb_size / 2, y0, cb_size / 2, cb_size, log2_cb_size, 1, idx - 1);
                break;
            case PART_2NxnU:
                hls_prediction_unit_cabac(s, x0, y0,               cb_size, cb_size     / 4, log2_cb_size, 0, idx);
                hls_prediction_unit_cabac(s, x0, y0 + cb_size / 4, cb_size, cb_size * 3 / 4, log2_cb_size, 1, idx);
                break;
            case PART_2NxnD:
                hls_prediction_unit_cabac(s, x0, y0,                   cb_size, cb_size * 3 / 4, log2_cb_size, 0, idx);
                hls_prediction_unit_cabac(s, x0, y0 + cb_size * 3 / 4, cb_size, cb_size     / 4, log2_cb_size, 1, idx);
                break;
            case PART_nLx2N:
                hls_prediction_unit_cabac(s, x0,               y0, cb_size     / 4, cb_size, log2_cb_size, 0, idx - 2);
                hls_prediction_unit_cabac(s, x0 + cb_size / 4, y0, cb_size * 3 / 4, cb_size, log2_cb_size, 1, idx - 2);
                break;
            case PART_nRx2N:
                hls_prediction_unit_cabac(s, x0,                   y0, cb_size * 3 / 4, cb_size, log2_cb_size, 0, idx - 2);
                hls_prediction_unit_cabac(s, x0 + cb_size * 3 / 4, y0, cb_size     / 4, cb_size, log2_cb_size, 1, idx - 2);
                break;
            case PART_NxN:
                hls_prediction_unit_cabac(s, x0,               y0,               cb_size / 2, cb_size / 2, log2_cb_size, 0, idx - 1);
                hls_prediction_unit_cabac(s, x0 + cb_size / 2, y0,               cb_size / 2, cb_size / 2, log2_cb_size, 1, idx - 1);
                hls_prediction_unit_cabac(s, x0,               y0 + cb_size / 2, cb_size / 2, cb_size / 2, log2_cb_size, 2, idx - 1);
                hls_prediction_unit_cabac(s, x0 + cb_size / 2, y0 + cb_size / 2, cb_size / 2, cb_size / 2, log2_cb_size, 3, idx - 1);
                break;
            }
        }
        if (!lc->cu.pcm_flag) {
            ...
            if (lc->cu.rqt_root_cbf) {
                lc_ca->cu.max_trafo_depth = lc->cu.pred_mode == MODE_INTRA ?
                                         s->sps->max_transform_hierarchy_depth_intra + lc_ca->cu.intra_split_flag :
                                         s->sps->max_transform_hierarchy_depth_inter;
                ret = hls_transform_tree_cabac(s, x0, y0, x0, y0, x0, y0,
                                         log2_cb_size,
                                         log2_cb_size, 0, 0);
                if (ret < 0)
                    return ret;
    ...
    x = y_cb * min_cb_width + x_cb;
    for (y = 0; y < length; y++) {
        memset(&s->qp_y_tab[x], lc_ca->qp_y, length);
        x += min_cb_width;
    }

    if(((x0 + (1<<log2_cb_size)) & qp_block_mask) == 0 &&
       ((y0 + (1<<log2_cb_size)) & qp_block_mask) == 0) {
        lc_ca->qPy_pred = lc_ca->qp_y;
    }

    set_ct_depth(s, x0, y0, log2_cb_size, lc_ca->ct.depth);

    SAMPLE_CBF(lc->ct.x[cb_depth], x0, y0)                         = lc->cu.x;
    SAMPLE_CBF(lc->ct.y[cb_depth], x0, y0)                         = lc->cu.y;
    ...
    SAMPLE_CBF(lc->ct.cu_transquant_bypass_flag[cb_depth], x0, y0) = lc->cu.cu_transquant_bypass_flag;
    return 0;
}
...
static int hls_coding_unit_compute(HEVCContext *s, int x0, int y0, int log2_cb_size, int cb_depth)
{
    ...
    int qp_block_mask    = (1<<(s->sps->log2_ctb_size - s->pps->diff_cu_qp_delta_depth)) - 1;
    int x, y, ret;
    lc->cu.x                         = SAMPLE_CBF(lc->ct.x[cb_depth], x0, y0);
    lc->cu.y                         = SAMPLE_CBF(lc->ct.y[cb_depth], x0, y0);
    ...
    lc->cu.cu_transquant_bypass_flag = SAMPLE_CBF(lc->ct.cu_transquant_bypass_flag[cb_depth], x0, y0);
    ...
    if (SAMPLE_CTB(s->skip_flag, x_cb, y_cb)) {
        ...
        if (lc->cu.pred_mode == MODE_INTRA) {
            ...
            case PART_2Nx2N:
                ...
                break;
            case PART_2NxN:
                hls_prediction_unit_compute(s, x0, y0,               cb_size, cb_size / 2, log2_cb_size, 0, idx);
                hls_prediction_unit_compute(s, x0, y0 + cb_size / 2, cb_size, cb_size / 2, log2_cb_size, 1, idx);
                break;
            case PART_Nx2N:
                hls_prediction_unit_compute(s, x0,               y0, cb_size / 2, cb_size, log2_cb_size, 0, idx - 1);
                hls_prediction_unit_compute(s, x0 + cb_size / 2, y0, cb_size / 2, cb_size, log2_cb_size, 1, idx - 1);
                break;
            case PART_2NxnU:
                hls_prediction_unit_compute(s, x0, y0,               cb_size, cb_size     / 4, log2_cb_size, 0, idx);
                hls_prediction_unit_compute(s, x0, y0 + cb_size / 4, cb_size, cb_size * 3 / 4, log2_cb_size, 1, idx);
                break;
            case PART_2NxnD:
                hls_prediction_unit_compute(s, x0, y0,                   cb_size, cb_size * 3 / 4, log2_cb_size, 0, idx);
                hls_prediction_unit_compute(s, x0, y0 + cb_size * 3 / 4, cb_size, cb_size     / 4, log2_cb_size, 1, idx);
                break;
            case PART_nLx2N:
                hls_prediction_unit_compute(s, x0,               y0, cb_size     / 4, cb_size, log2_cb_size, 0, idx - 2);
                hls_prediction_unit_compute(s, x0 + cb_size / 4, y0, cb_size * 3 / 4, cb_size, log2_cb_size, 1, idx - 2);
                break;
            case PART_nRx2N:
                hls_prediction_unit_compute(s, x0,                   y0, cb_size * 3 / 4, cb_size, log2_cb_size, 0, idx - 2);
                hls_prediction_unit_compute(s, x0 + cb_size * 3 / 4, y0, cb_size     / 4, cb_size, log2_cb_size, 1, idx - 2);
                break;
            case PART_NxN:
                hls_prediction_unit_compute(s, x0,               y0,               cb_size / 2, cb_size / 2, log2_cb_size, 0, idx - 1);
                hls_prediction_unit_compute(s, x0 + cb_size / 2, y0,               cb_size / 2, cb_size / 2, log2_cb_size, 1, idx - 1);
                hls_prediction_unit_compute(s, x0,               y0 + cb_size / 2, cb_size / 2, cb_size / 2, log2_cb_size, 2, idx - 1);
                hls_prediction_unit_compute(s, x0 + cb_size / 2, y0 + cb_size / 2, cb_size / 2, cb_size / 2, log2_cb_size, 3, idx - 1);
                break;
            }
        ...
        if (!lc->cu.pcm_flag) {
            if (lc->cu.rqt_root_cbf) {
                ret = hls_transform_tree_compute(s, x0, y0, x0, y0, x0, y0,
                                         log2_cb_size,
                                         log2_cb_size, 0, 0);
                if (ret < 0)
                    return ret;
                               ...
                               int log2_cb_size, int cb_depth)
{
    ...
    if (s->pps->cu_qp_delta_enabled_flag &&
        ...
        lc_ca->tu.is_cu_qp_delta_coded = 0;
        lc_ca->tu.cu_qp_delta          = 0;
    }
                                 ...
                                 int ctb_addr_ts)
{
    ...
    if (s->pps->entropy_coding_sync_enabled_flag) {
        if (x_ctb == 0 && (y_ctb & (ctb_size - 1)) == 0)
            lc_ca->first_qp_group = 1;
        lc_co->end_of_tiles_x = s->sps->width;
        ...
        if (ctb_addr_ts && s->pps->tile_id[ctb_addr_ts] != s->pps->tile_id[ctb_addr_ts - 1]) {
            ...
            lc_co->end_of_tiles_x   = x_ctb + (s->pps->column_width[idxX] << s->sps->log2_ctb_size);
            lc_ca->first_qp_group   = 1;
        }
    } else {
        lc_co->end_of_tiles_x = s->sps->width;
    ...
    lc_co->end_of_tiles_y = FFMIN(y_ctb + ctb_size, s->sps->height);
    ...
    lc_co->slice_or_tiles_left_boundary = slice_left_boundary + (tile_left_boundary << 1);
    lc_co->slice_or_tiles_up_boundary   = slice_up_boundary   + (tile_up_boundary   << 1);
    lc_co->ctb_left_flag = ((x_ctb > 0) && (ctb_addr_in_slice > 0)                  && !tile_left_boundary);
    lc_co->ctb_up_flag   = ((y_ctb > 0) && (ctb_addr_in_slice >= s->sps->ctb_width) && !tile_up_boundary);
    lc_co->ctb_up_right_flag = ((y_ctb > 0)                 && (ctb_addr_in_slice+1 >= s->sps->ctb_width) && (s->pps->tile_id[ctb_addr_ts] == s->pps->tile_id[s->pps->ctb_addr_rs_to_ts[ctb_addr_rs+1 - s->sps->ctb_width]]));
    lc_co->ctb_up_left_flag  = ((x_ctb > 0) && (y_ctb > 0)  && (ctb_addr_in_slice-1 >= s->sps->ctb_width) && (s->pps->tile_id[ctb_addr_ts] == s->pps->tile_id[s->pps->ctb_addr_rs_to_ts[ctb_addr_rs-1 - s->sps->ctb_width]]));

    lc_ca->ctb_left_flag = lc_co->ctb_left_flag;
    lc_ca->ctb_up_flag   = lc_co->ctb_up_flag;
}

static void hls_decode_neighbour_cabac(HEVCContext *s, int x_ctb, int y_ctb,
                                 int ctb_addr_ts)
{
    HEVCLocalContextCabac   *lc_ca = &s->HEVClc->ca;
    int ctb_size          = 1 << s->sps->log2_ctb_size;
    ...
    lc_ca->ctb_left_flag = ((x_ctb > 0) && (ctb_addr_in_slice > 0)                  && !tile_left_boundary);
    lc_ca->ctb_up_flag   = ((y_ctb > 0) && (ctb_addr_in_slice >= s->sps->ctb_width) && !tile_up_boundary);

}

static void hls_decode_neighbour_compute(HEVCContext *s, int x_ctb, int y_ctb,
                                 int ctb_addr_ts)
{
    HEVCLocalContextCompute  *lc_co =  &s->HEVClc->co;
    int ctb_size          = 1 << s->sps->log2_ctb_size;
...
static int hls_coding_entry_cabac(HEVCContext *s, int ctb_addr_ts, int ctb_size)
{
    ...
    more_data = hls_coding_quadtree_cabac(s, x_ctb, y_ctb, s->sps->log2_ctb_size, 0);
    if (more_data >= 0) {
        ff_hevc_save_states(s, ctb_addr_ts+1);
    }
    return more_data;
}
...
static int hls_slice_data(HEVCContext *s, const uint8_t *nal, int length)
{
    ...
    for (i = 1; i < s->threads_number; i++) {
        s->sList[i]->HEVClc->ca.first_qp_group = 1;
...
static int decode_nal_units(HEVCContext *s, const uint8_t *buf, int length)
{
    ...
    while (length >= 4) {
        ...
        ret = init_get_bits8(&s->HEVClc->ca.gb, nal->data, nal->size);
        if (ret < 0)
            goto fail;
        ret = hls_nal_unit(s);
