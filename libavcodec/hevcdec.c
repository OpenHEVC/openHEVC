...
//    const int phaseAlignFlag = 0; // TO DO ((HEVCVPS*)s->ps.vps_list[s->ps.sps->ps.vps_id]->data)->phase_align_flag;
    ...
    else if(s->up_filter_inf.scaleXLum == 32768 && s->up_filter_inf.scaleYLum == 32768)
        s->up_filter_inf.idx = X2;
    else if(s->up_filter_inf.scaleXLum == 43691 && s->up_filter_inf.scaleYLum == 43691)
...
#if OHCONFIG_UPSAMPLING360
    {
        ...
        const int widthEL_v[2]  = {current_layer_window.pic_width_vps_in_luma_samples  - current_layer_window.conf_win_vps_left_offset   - current_layer_window.conf_win_vps_right_offset,   widthEL>>1};
        const int heightEL_v[2] = {current_layer_window.pic_height_vps_in_luma_samples - current_layer_window.conf_win_vps_bottom_offset - current_layer_window.conf_win_vps_top_offset, heightEL>>1};
            ...
            /*Luma*/
            {
                ...
                for (int j = 0; j < heightEL_v[0]; j++){
                    for (int i = 0; i < widthEL_v[0]; i++) {
                        const int el_pos = j* scaled_ref_layer_window.pic_width_vps_in_luma_samples+ i;
...
//if (s->ps.sps_list[s->ps.pps->sps_id]!=NULL)
    ...
    if (!sh->dependent_slice_segment_flag) {
        ...
        if(s->ps.pps->num_extra_slice_header_bits > i) {
        ...
        for(;i < s->ps.pps->num_extra_slice_header_bits;++i){
