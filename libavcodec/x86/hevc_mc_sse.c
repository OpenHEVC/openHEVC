...
#define WEIGHTED_LOAD2_1()                                                     \
    __m128i r5 = _mm_loadl_epi64((__m128i *) &src2[x    ])
#define WEIGHTED_LOAD4_1()                                                     \
    WEIGHTED_LOAD2_1()
#define WEIGHTED_LOAD8_1()                                                     \
    __m128i r5 = _mm_load_si128((__m128i *) &src2[x    ])
#define WEIGHTED_LOAD16_1()                                                    \
    WEIGHTED_LOAD8_1();                                                        \
    __m128i r6 = _mm_load_si128((__m128i *) &src2[x + 8])
...
#define BI_WEIGHTED_INIT(D)                                                    \
    ...
    const __m128i ox0    = _mm_set1_epi32(_ox0 << (D - 8));                    \
    const __m128i ox1    = _mm_set1_epi32(_ox1 << (D - 8));                    \
    const __m128i wx0    = _mm_set1_epi16(_wx0);                               \
    const __m128i wx1    = _mm_set1_epi16(_wx1);                               \
    const __m128i offset = _mm_set1_epi32( (((_ox0 + _ox1) << (D - 8)) + 1) << (shift2 - 1));\
    DST_INIT_ ## D()
...
void ff_hevc_put_hevc_bi_qpel_hv ## H ## _ ## D ## _sse (                      \
    ...
    ff_hevc_put_hevc_bi_qpel_v ## H ## _14_ ## D ## _sse(                      \
        dst, dststride, (uint8_t *) tmp, MAX_PB_SIZE <<1 , src2, src2stride, width, height, mx, my);\
}
