...
static void derive_spatial_merge_candidates(HEVCContext *s, int x0, int y0, int nPbW, int nPbH, int log2_cb_size, int singleMCLFlag, int part_idx,  struct MvField mergecandlist[])
{
    ...
    if(cand_up_right && check_B0 && !(tab_mvf[(yB0_pu) * pic_width_in_min_pu + xB0_pu].is_intra)) {
        isAvailableB0 = 1;
    } else {
        isAvailableB0 = 0;
    }
    ...
    if(cand_bottom_left && check_A0 && !(tab_mvf[(yA0_pu) * pic_width_in_min_pu + xA0_pu].is_intra)) {
        isAvailableA0 = 1;
    } else {
        isAvailableA0 = 0;
    }
