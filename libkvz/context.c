...
/**
 * \brief Initialize cabac context to be used for coding
 * \param cabac cabac context struct
 * \param slice type of slice we are coding (P/B/I)
 */
...
void kvz_init_cabac_contexts(cabac_data_t *cabac, int8_t QP, int8_t slice)
{
  ...
  for (i = 0; i < 27; i++)
  {
    kvz_ctx_init(&cabac->ctx.cu_sig_model_luma[i], QP, INIT_SIG_FLAG[slice][i]);
    if (i < 15)
      kvz_ctx_init(&cabac->ctx.cu_sig_model_chroma[i], QP, INIT_SIG_FLAG[slice][i + 27]);
  }
}
                                ...
                                int32_t pos_y, int32_t block_type, int8_t texture_type)
{
  ...
  offset = (block_type == 3) ? ((scan_idx == SCAN_DIA) ? 9 : 15) : ((texture_type == 0) ? 21 : 12);
